...
export abstract class BaseRepository<T> {
    ...
    /**
     * Finds any record in the database table that matches all the given identifiers.
     * If multiple records match an error is thrown.
     * If no identifier is provided, throws an error.
     ...
     * @returns A promise that resolves to the found row, or null if no row is found.
     */
    protected async find<K extends keyof T>(identifier: Partial<T>, fields?: K[]): Promise<Pick<T, K> | null> {
        ...
        const sql = `SELECT ${fields && fields.length > 0 ? fields.join(', ') : '*'} FROM ${this.tableName} WHERE ${whereField.join(' AND ')}`;
     ...
     */
    protected async update(identifier: Partial<T>, data: Partial<Omit<T, 'id'>>): Promise<number> {
        ...
        const sql = `UPDATE ${this.tableName} SET ${setFields.join(', ')} WHERE ${whereField.join(' AND ')}`;
    ...
    protected async delete(identifier: Partial<T>): Promise<number> {
        ...
        const sql = `DELETE FROM ${this.tableName} WHERE ${whereField.join(' AND ')}`;
